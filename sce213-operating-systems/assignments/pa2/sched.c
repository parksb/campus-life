...
void dump_status(void) {
    ...
    if (current) {
        printf("%2d (%s): %d + %d/%d at %d\n", current->pid,
               __process_status_sz[current->status], current->__starts_at,
    ...
    list_for_each_entry(p, &readyqueue, list) {
        printf("%2d (%s): %d + %d/%d at %d\n", p->pid,
               __process_status_sz[p->status], p->__starts_at, p->age,
               p->lifespan, p->prio);
    }
...
static void __briefing_process(struct process *p) {
    ...
    printf("- Process %d: Forked at tick %d and run for %d tick%s with initial "
           "priority %d\n",
           p->pid, p->__starts_at, p->lifespan, p->lifespan >= 2 ? "s" : "",
           p->prio);
