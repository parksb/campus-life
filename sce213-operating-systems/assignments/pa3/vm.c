...
extern bool translate(enum memory_access_type rw, unsigned int vpn,
                      unsigned int *pfn);
...
static void __do_simulation(FILE *input) {
    ...
    while (fgets(command, sizeof(command), input)) {
        ...
        if (nr_tokens == 1) {
            ...
            } else if (strmatch(tokens[0], "read") ||
                       strmatch(tokens[0], "r")) {
                if (!__access_memory(READ, strtoimax(tokens[1], NULL, 0)))
                    break;
                ;
            } else if (strmatch(tokens[0], "write") ||
                       strmatch(tokens[0], "w")) {
                if (!__access_memory(WRITE, strtoimax(tokens[1], NULL, 0)))
                    break;
                ;
            } else {
                printf("Unknown command %s\n", tokens[0]);
...
int main(int argc, char *argv[]) {
    ...
    if (!quiet && !argv[optind]) {
        ...
        printf(" \\ \\    / /  \\/  |  / ____(_)               | |     | |\n");
        printf("  \\ \\  / /| \\  / | | (___  _ _ __ ___  _   _| | __ _| |_ "
               "___  _ "
               "__ \n");
        printf(
            "   \\ \\/ / | |\\/| |  \\___ \\| | '_ ` _ \\| | | | |/ _` | __/ _ "
